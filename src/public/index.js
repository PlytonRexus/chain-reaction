...
function checkAndPlay(event) {
    ...
    */
    ...
    if (isAlreadyOut()) {
        gameState.turns += 1;
        socket.emit("send_game_loop", { gameState, time: Date.now() });
        switchTurn();
        return;
    }
    if (isRightCell(...coords)) {
        ...
        socket.emit("send_game_loop", { 
            gameState, time: Date.now(), status: stt
        });
...
function resetGame(dontAsk) {
    ...
    if (reset) socket.emit("send_game_reset", { time: Date.now() });
}
...
function createAndLoadElements() {
    ...
    elements.syncButton = ce('button', {
        class: "button settings-button",
        id: "sync-button"
    }, null, {
        innerHTML: "Synchronise",
        onclick: function(e) {
            // for state synchronisation
            socket.emit("send_sync_request", { username: state.username });
        }
    });
    ...
    elements.settings = ce('div', {
        ...
        elements.undoButton, elements.syncButton, 
        elements.resetButton, elements.turnInfo
    ]);
...
function loadGrid() {
    ...
    addToParent(
        ...
        elements.gc, elements.settings, elements.chatbox
    );
}
